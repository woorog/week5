...
void levelOrderTraversal(BSTNode* root)
{
	...
	tree_q.head = NULL;
	tree_q.tail = NULL;
	enqueue(&tree_q.head, &tree_q.tail, root);

	while (!isEmpty(tree_q.head)) {
		cur = dequeue(&tree_q.head, &tree_q.tail);
		printf("%d ", cur->item);
		if (cur->left != NULL)
			enqueue(&tree_q.head, &tree_q.tail, cur->left);
		if (cur->right != NULL)
			enqueue(&tree_q.head, &tree_q.tail, cur->right);
	}
}
